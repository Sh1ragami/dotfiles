...
TARGET_SPECIAL="special:gemini"

# 1. すでに special:gemini に紛れ込んだ他のウィンドウを現在のワークスペースへ救出
ACTIVE_WS=$(hyprctl activeworkspace -j | jq -r '.id')
hyprctl clients -j | jq -r ".[] | select(.workspace.name == \"$TARGET_SPECIAL\" and .class != \"$GEMINI_CLASS\") | .address" | while read -r addr; do
...
# 3. special:gemini が現在画面上に表示されているか判定
...
if [ "$WAS_LAUNCHED" = true ]; then
    ...
    if [ -n "$IS_SPECIAL_OPEN" ]; then
        hyprctl dispatch focuswindow "class:^${GEMINI_CLASS}$" >/dev/null 2>&1
    fi
    hyprctl dispatch togglespecialworkspace gemini
fi
